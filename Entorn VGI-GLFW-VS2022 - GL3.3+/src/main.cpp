...
// PROJECCIÓ PERSPECTIVA
		...
		// Activar shader Visualització Escena
...
// ----------- POP UP Fitxers
		...
		case GLFW_KEY_F1:
			...
			nomfitxer = nom.c_str();	// Conversió tipus string --> char *

			// Conversió std::string --> char *
			nomfitx = new char[nom.length() + 1];
			strcpy(nomfitx, nom.c_str());
			// Entorn VGI: Llegir fitxer fractal (nomfitx) i inicialitzar alçades
...
// ----------- POP UP ILUMINACIÓ
	...
	case GLFW_KEY_F5:
		...
		//glUniform1i(glGetUniformLocation(shader_programID, "sw_intensity[0]"), sw_material[0]); // Pas màscara llums al shader
		break;

	// Tecla Iluminació --> Reflexió Material -> Ambient?
	case GLFW_KEY_F6:
		sw_material[1] = !sw_material[1];
		//glUniform1i(glGetUniformLocation(shader_programID, "sw_intensity[1]"), sw_material[1]); // Pas màscara llums al shader
		break;

	// Tecla Iluminació --> Reflexió Material -> Difusa?
	case GLFW_KEY_F7:
		sw_material[2] = !sw_material[2];
		//glUniform1i(glGetUniformLocation(shader_programID, "sw_intensity[2]"), sw_material[2]); // Pas màscara llums al shader
		break;

	// Tecla Iluminació --> Reflexió Material -> Especular?
	case GLFW_KEY_F8:
		sw_material[3] = !sw_material[3];
		//glUniform1i(glGetUniformLocation(shader_programID, "sw_intensity[3]"), sw_material[3]); // Pas màscara llums al shader
		break;
...
// ----------- POP UP Llums
	...
	case GLFW_KEY_A:
		...
		sw_il = true;
		if (shader_menu != CAP_SHADER) glUniform1i(glGetUniformLocation(shader_programID, "sw_intensity[1]"), (llum_ambient && sw_material[1])); // Pas màscara llums al shader
		break;
	...
	case GLFW_KEY_0:
		...
		sw_il = true;
		if (shader_menu != CAP_SHADER) glUniform1i(glGetUniformLocation(shader_programID, "sw_lights[0]"), llumGL[0].encesa); // Pas màscara Llum #0 al shader
		break;
