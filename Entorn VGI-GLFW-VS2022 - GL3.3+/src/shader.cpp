...
//******** Entorn bàsic VS2019 MULTIFINESTRA amb OpenGL, interfície GLFW i Status Bar
...
*      Loads custom shader by specifying filename (expects frag/vert pair)
*****************************************************************************/
GLuint Shader::initializeShaders(std::string filename)
{
	GLuint v, f;
	char* vs = NULL, * fs = NULL;
	//CString file_Vert, file_Frag;
	std::string file_Vert, file_Frag;
	...
	if ((vs != NULL) && (fs != NULL))
	{
		...
		glAttachShader(this->programID, v);	// Relaciona Vertex Shader al Program
		glAttachShader(this->programID, f);	// relaciona Fragment Shader al Program

		// Attribute Locations must be setup before calling glLinkProgram()
		glBindAttribLocation(this->programID, 0, "in_Vertex"); // Vèrtexs
		glBindAttribLocation(this->programID, 1, "in_Normal"); // Normals
		glBindAttribLocation(this->programID, 2, "in_TexCoord"); // Textura
		glBindAttribLocation(this->programID, 3, "in_Color"); // Color
